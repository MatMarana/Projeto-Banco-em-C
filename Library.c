...
int verificaCliente(int numeroClientes, char cpf[15], int senha, int indice){
  if(strcmp(dados[indice].cpf,cpf) == 0){
    if(dados[indice].senha == senha){
        return 1;
    }
...
void apagarCliente(int numeroClientes,char cpf[15]){ 
  ...
  for (; i < numeroClientes - 1; i++) { // apaga o clietente com o cpf iguial a variavel i
      dados[i] = dados[i + 1];
  }
}
...
void debito(int numeroClientes, char cpf[15], int senha){ 
  ...
  for(int i = 0; i < numeroClientes; i++){
    if (verificaCliente(numeroClientes,cpf,senha,i) == 1){ //compara os cpfs do array de structs com o digitado
